...
The watchdog never deletes Docker volumes, database files, images, or application data. It only runs targeted `restart` and `up -d --force-recreate` commands for services listed in `WATCHDOG_SERVICES`.
...
`WATCHDOG_SERVICES` is a whitespace-separated list of production Compose services the watchdog may restart or recreate. The default is `api`, so optional profile services are not started unless explicitly configured.

Tunnel deployments that start the `cloudflared` profile can opt in to the previous two-service behavior:
...
2. If the check fails, restarts only the services listed in `WATCHDOG_SERVICES`. With the default configuration:
...
4. If the check still fails, recreates only the same configured services. With the default configuration:
...
If `WATCHDOG_SERVICES="api cloudflared"` is configured, the recovery commands target both services. This is intended only for hosts that also run:
